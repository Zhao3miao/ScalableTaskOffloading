...
class BlindIPPOAgent:
    ...
    def __init__(self, config):
        ...
        self.n_servers = config.get("n_servers", (self.obs_dim - 6) // 3)
        self.blind_obs_dim = 6 + self.n_servers  # Global ratio + 5 local + K channel
        ...
        self.critic = nn.Sequential(
            nn.Linear(self.blind_obs_dim, hidden_size_critic),
            nn.Tanh(),
    ...
    def predict(self, obs, active_mask=None):
        ...
        # Blind Masking: zero out global ratio, and slice out congestions
        obs_tensor[:, 0] = 0.0
        obs_tensor = obs_tensor[:, :self.blind_obs_dim]
